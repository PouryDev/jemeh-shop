<x-admin.layout :title="'ایجاد رنگ'">
    <div class="flex items-center gap-2 mb-4">
        ...
        <h1 class="text-xl font-bold">ایجاد رنگ</h1>
    </div>
    
    <form method="POST" action="{{ route('admin.colors.store') }}" class="space-y-6">
        @csrf
                ...
                @if(old('hex_code'))
                    <div class="p-4 bg-white/5 rounded-lg">
                        ...
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 rounded-full border border-white/20" style="background-color: {{ old('hex_code') }}"></div>
                            <div class="text-sm text-gray-300">{{ old('name') ?: 'نام رنگ' }}</div>
                        </div>
                    </div>
                @endif
            </div>
